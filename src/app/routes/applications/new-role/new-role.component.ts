...
export class NewRoleComponent implements OnInit, AfterViewInit {
  ...
  private TOASTR_HEADER = $localize`Create New Role`;
  public txs: any[];
  ...
  private _init(data: any) {
    if (data && data.viewType) {
      ...
      if (this.viewType === ViewType.UPDATE && data.origData) {
        ...
        this.TOASTR_HEADER = $localize`Update Role`;
  ...
  async confirmParentNamespace() {
    if (this.roleForm.value.parentNamespace) {
      try {
        ...
        if (exists) {
          ...
          if (exists) {
            ...
            if (!isOwner) {
              this.toastr.error(
                $localize`You are not authorized to create a role under this namespace.`,
                this.TOASTR_HEADER
              );
            ...
            this.toastr.error(
              $localize`Role subdomain in this namespace does not exist.`,
              this.TOASTR_HEADER
            );
            this.dialog.closeAll();
          }
      ...
      this.toastr.error(
        $localize`Parent Namespace is missing.`,
        this.TOASTR_HEADER
      );
      this.dialog.closeAll();
    }
  ...
  async closeDialog(isSuccess?: boolean) {
    if (this.roleForm.touched && !isSuccess) {
      ...
      if (isSuccess) {
        if (this.origData) {
          this.toastr.success(
            $localize`Role is successfully updated.`,
            this.TOASTR_HEADER
          );
        } else {
          this.toastr.success(
            $localize`Role is successfully created.`,
            this.TOASTR_HEADER
          );
        }
      }
