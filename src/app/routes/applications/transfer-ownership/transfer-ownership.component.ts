...
export class TransferOwnershipComponent implements OnDestroy {
  ...
  async submit() {
    if (this.newOwnerDID === this.iamService.signerService.did) {
      this.toastr.error(
        $localize`You cannot transfer to your own DID.`,
        TOASTR_HEADER
      );
      ...
      if (this.namespace) {
        ...
        this.toastr.error($localize`Invalid action.`, TOASTR_HEADER);
      }
    }
  }
  ...
  private async _transferOrgAppRole() {
    if (
      await this.confirm(
        $localize`You will no longer be the owner of this namespace. Do you wish to continue?`
      )
    ) {
  ...
  private async _transferAsset() {
    if (
      await this.confirm(
        $localize`You will no longer be the owner of this asset. Do you wish to continue?`
      )
    ) {
      this.loadingService.show(
    ...
    if (steps) {
      for (let index = startIndex; index < steps.length; index++) {
        ...
        try {
          ...
          this.toastr.info(
            step.info,
            $localize`Transaction Success (${index + 1}/${steps.length})`
          );
