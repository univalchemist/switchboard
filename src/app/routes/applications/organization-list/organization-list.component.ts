  ...
  getTooltip(element: any) {
    ...
    if (element.subOrgs && element.subOrgs.length) {
      ...
      if (element.subOrgs.length > 1) {
        retVal = $localize`Sub-Organizations \n`;
      } else {
        retVal = $localize`Sub-Organization \n`;
      }
  ...
  private async getRemovalSteps(roleDefinition: any) {
    ...
    try {
      return returnSteps
        ...
        : [
            {
              info: $localize`Confirm removal in your safe wallet`,
              next: async () => await call,
            },
          ];
    } catch (e) {
      console.error(e);
      this.toastr.error(e, $localize`Delete Organization`);
    } finally {
