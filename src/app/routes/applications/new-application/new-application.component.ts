  ...
  implements AfterViewInit
{
  ...
  private TOASTR_HEADER = $localize`Create New Application`;
  ...
  private prepareOriginalData() {
    if (this.data && this.data.viewType && this.isUpdating) {
      this.TOASTR_HEADER = $localize`Update Application`;
    }
  }
  ...
  async createNewApp(data: AppCreationDefinition) {
    ...
    if (exists) {
      ...
      if (!isOwner) {
        ...
        this.toastr.error(
          $localize`Application namespace already exists. You have no access rights to it.`,
          this.TOASTR_HEADER
        );
      } else {
        ...
        if (
          !(await this.confirm(
            $localize`Application namespace already exists. Do you wish to continue?`
          ))
        ) {
          allowToProceed = false;
        } else {
  ...
  async updateApp(data: AppCreationDefinition) {
    ...
    if (!isOwner) {
      ...
      if (
        !(await this.confirm(
          $localize`You are updating details of this application. Do you wish to continue?`
        ))
      ) {
        allowToProceed = false;
      } else {
  ...
  private async proceedUpdateStep(req: any, skipNextStep?: boolean) {
    try {
      ...
      if (this.stepper.selectedIndex !== 3 && retryCount === this._retryCount) {
        // Move to Complete Step
        this.toastr.info(
          $localize`Set definition for application`,
  ...
  async closeDialog(isSuccess?: boolean) {
    if (isSuccess) {
      if (this.origData) {
        this.toastr.success(
          $localize`Application is successfully updated.`,
          this.TOASTR_HEADER
        );
      } else {
        this.toastr.success(
          $localize`Application is successfully created.`,
          this.TOASTR_HEADER
        );
      }
    }
