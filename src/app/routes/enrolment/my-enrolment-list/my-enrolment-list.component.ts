    ...
    const dialogRef = this.dialog
      .open(ConfirmationDialogComponent, {
        ...
        data: {
          ...
          message: $localize`Are you sure to cancel this enrolment request?`,
        },
        maxWidth: '100%',
        disableClose: true,
      })
    ...
    if (await dialogRef) {
      ...
      try {
        ...
        this.toastr.success(
          ...
          $localize`Cancel Enrolment Request`
        );
        this.updateList();
      } catch (e) {
        console.error(e);
        this.toastr.error(
          $localize`Failed to cancel the enrolment request.`,
          TOASTR_HEADER
        );
      } finally {
  ...
  private defineColumns() {
    this.columns = [
      ...
      },
      {
        ...
        field: 'status',
        header: $localize`Issuance Status`,
        customElement: this.status,
      },
      {
        type: ColumnType.String,
        field: 'expirationStatus',
        header: $localize`Expiration Status`,
      },
      {
        type: ColumnType.Date,
        field: 'expirationDate',
        header: $localize`Expiration Date`,
      },
      {
