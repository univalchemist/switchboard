...
export class AssetListComponent implements OnInit, OnDestroy {
  ...
  subscribeTo(source: Observable<AssetList[]>) {
    return source.pipe(finalize(() => this.loadingService.hide())).subscribe(
      ...
      (error) => {
        ...
        this.toastr.error(
          error.message ||
            $localize`Could not retrieve list. Please contact system administrator.`
        );
      }
    );
  }
  ...
  transferOwnership(type: any, data: Asset) {
    const dialogRef = this.dialog
      ...
      .subscribe((res: any) => {
        if (res) {
          this.toastr.success(
            $localize`Asset is offered successfully.`,
  ...
  async cancelAssetOffer(data: Asset) {
    if (
      await this._confirm(
        $localize`The offered ownership of this asset will be cancelled.`,
      ...
      try {
        ...
        this.toastr.success(
          $localize`Offered ownership is cancelled successfully.`,
          HEADER_CANCEL_OWNERSHIP
        );
        this.getAssetList();
      } catch (e) {
        ...
        this.toastr.error(
          e.message ||
            $localize`A system error has occured. Please contact system administrator.`,
          HEADER_CANCEL_OWNERSHIP
        );
      } finally {
  ...
  async approveAssetOffer(data: Asset) {
    if (
      await this._confirm(
        $localize`You will become the owner of this asset.`,
      ...
      try {
        ...
        this.toastr.success(
          $localize`A new asset is added successfully to your list.`,
          HEADER_ACCEPT_OWNERSHIP
        );
  ...
  async rejectAssetOffer(data: Asset) {
    if (
      await this._confirm(
        $localize`You are rejecting this offered asset.`,
      ...
      try {
        ...
        this.toastr.success(
          $localize`You have rejected an offered asset successfully.`,
          HEADER_REJECT_OWNERSHIP
        );
        this.getAssetList();
        this.notifService.decreaseAssetsOfferedToMeCount();
  ...
  generateQrCode(data: Asset): void {
    this.qrCodeService.open({
      header: $localize`Asset DID QR-Code`,
      qrCodeData: {
        type: ScanType.Asset,
        data: {
          did: data.id,
