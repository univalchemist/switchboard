...
export class VerificationMethodComponent implements OnInit {
  ...
  getPublicKeyErrorMsg() {
    ...
    if (this.publicKey.hasError('isHexInvalid')) {
      return $localize`Invalid input. Public key must start with "0x" to be followed by 66 or 130 Hexadecimal characters.`;
    }

    if (this.publicKey.hasError('listContains')) {
      return $localize`Public key entered already exists, please choose another.`;
    }
