...
export class IamService {
  ...
  getAssetById(id: string) {
    return this.wrapWithLoadingService(
      ...
      { message: $localize`Getting selected asset data...` }
    );
  }
  ...
  async initializeConnection({
    ...
    try {
      ...
      if (initCacheServer) {
        ...
        if (!this.domainsService) {
          this.sentryService.info(
            $localize`domains service is not initialized, assetsService: ${Boolean(
              assetsService
            )}, cacheClient: ${Boolean(
              cacheClient
      ...
      throw {
        did: undefined,
        connected: false,
        userClosedModal: e.message === $localize`User closed modal`,
